...
const Units = () => {
  const { setStep, setUnitsCount, unitsForm, setUnitsForm } =
  ...
  const onSubmit: SubmitHandler<UnitsForm> = (data) => {
    ...
    // Duplicates
    const duplicateNames = unitNames.filter(
      (unit, unitIndex) => unitNames.indexOf(unit) !== unitIndex
    );
          ...
          fields.map((field, itemIndex) => (
            <HStack align="center" key={field.id}>
              <FormControl>
                ...
                <Input
                  {...register(
                    `units.${unitIndex}.bedrooms.${itemIndex}.bedroomNumber`
                  )}
                  size="sm"
                  disabled
                  type="number"
              ...
              <FormControl>
                ...
                <ChakraSelect
                  size="sm"
                  {...register(
                    `units.${unitIndex}.bedrooms.${itemIndex}.enSuite`
                  )}
                >
              ...
              <FormControl>
                ...
                <ChakraSelect
                  size="sm"
                  {...register(
                    `units.${unitIndex}.bedrooms.${itemIndex}.master`
                  )}
                >
                  <option value="yes">Yes</option>
                  <option value="no">No</option>
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <Accordion h="100%">
              ...
              <AccordionItem w="100%" mb={4} gap={2} key={unit.id}>
                <AccordionButton>
                  <Box as="span" flex="1" textAlign="left">
                    Unit {`${unitIndex + 1}`}
                    {Boolean(errors?.units?.[unitIndex]) && (
                ...
                <AccordionPanel>
                  ...
                  >
                    <FormControl
                      isInvalid={Boolean(
                        (errors?.units?.[unitIndex] as { name: object })?.name
                      )}
                    >
                    ...
                    <FormControl
                      isInvalid={Boolean(errors?.units?.[unitIndex]?.type)}
                    >
                      <FormLabel>Type</FormLabel>
                      <ChakraSelect
                        size="xs"
                        {...register(`units.${unitIndex}.type`)}
                      ...
                      {errors.units?.[unitIndex]?.type != null && (
                        <FormErrorMessage>
                          {
                            (
                              errors.units?.[unitIndex]?.type as Partial<{
                                message: string;
                              }>
                            )?.message
                          }
                    ...
                    <FormControl
                      isInvalid={Boolean(
                        (errors?.units?.[unitIndex] as { baths: object })?.baths
                      )}
                    >
                      ...
                      />
                      {(errors.units?.[unitIndex] as { baths: object })
                        ?.baths != null && (
                        <FormErrorMessage>
                          {
                            (
                              errors.units?.[unitIndex] as {
                                baths: Partial<{ message: string }>;
                              }
                            )?.baths?.message
                          }
                        </FormErrorMessage>
                  ...
                  >
                    <FormControl>
                      ...
                      />
                      <FormHelperText>
                        Amenities offered by this unit
                      </FormHelperText>
                    </FormControl>
                    <FormControl
                      isInvalid={Boolean(
                        (errors?.units?.[unitIndex] as { price: object })?.price
                      )}
                    >
                      ...
                      <Input
                        ...
                        size="xs"
                      />
                      {(errors.units?.[unitIndex] as { price: object })
                        ?.price != null && (
                        <FormErrorMessage>
                          {
                            (
                              errors.units?.[unitIndex] as {
                                price: Partial<{ message: string }>;
                              }
                            )?.price?.message
                          }
                        </FormErrorMessage>
        ...
        <Box mt={5}>
          <Button size="sm" onClick={appendUnit} colorScheme="green">
            Add Unit
          </Button>
        </Box>
      </Accordion>
